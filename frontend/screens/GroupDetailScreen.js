  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await groupApi(
        `/api/groups/${group.groupId}?userId=${user.userId}`
      );
    ...
    try {
      ...
      await groupApi(`/api/groups/${group.groupId}/invitations`, {
        ...
        body: JSON.stringify({
          ...
          inviteeLoginId: inviteId.trim(),
        }),
      });

      showAlert(
        '초대 완료',
        `${inviteId.trim()} 사용자에게 초대를 보냈습니다.`
      );
        ...
        <View style={styles.card}>
          ...
          {isLeader ? (
            <SecondaryButton
              ...
              onPress={() => {
                setDraftDistrict(regionSigungu);
                setRegionOpen(true);
              }}
          ...
          {inviteOpen ? (
            <View style={styles.inviteBox}>
              <TextInput
                ...
                placeholder="상대방의 로그인 아이디"
                placeholderTextColor={colors.textFaint}
                autoCapitalize="none"
              />

              <PrimaryButton
                title={inviting ? '전송 중' : '초대'}
                onPress={invite}
      ...
      >
        ...
        >
          ...
          >
            <View
              style={
                styles.regionSheetHeader
              }
            >
              <Text
                style={styles.regionSheetTitle}
              >
                활동 구·군 선택
              </Text>
