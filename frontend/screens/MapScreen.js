...
export default function MapScreen({ onBack, onLocationClick, locations, setLocations }) {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.desc}>{optimizing ? '최적 경로 계산 중' : optimized ? '최적 방문 순서 안내' : '방문지를 확인하고 경로를 실행하세요'}</Text>
        </View>
      ...
      <View style={styles.mapArea}>
        ...
        {optimizing && (
          <View style={styles.loadingOverlay}>
            ...
            <Text style={styles.loadingDesc}>현재 위치 기준 최단 방문 순서를 계산합니다.</Text>
          </View>
        )}
      </View>
        ...
        <TouchableOpacity style={styles.modalBg} activeOpacity={1} onPress={() => setSelected(null)}>
          <TouchableOpacity activeOpacity={1} style={styles.sheet}>
            ...
            <View style={styles.placeRow}>
              <View style={styles.placeIcon}><Text>📍</Text></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.placeName}>{selected?.name}</Text>
                <Text style={styles.placeAddr}>{selected?.address}</Text>
              </View>
            </View>
            ...
            <View style={styles.actionRow}>
              {[
                ...
                <TouchableOpacity key={b.type} style={styles.sheetButton} onPress={() => { const loc = selected; setSelected(null); onLocationClick?.(loc, b.type); }}>
